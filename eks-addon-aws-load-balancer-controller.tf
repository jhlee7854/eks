data "aws_iam_policy_document" "AmazonEKSLoadBalancerControllerAssumeRolePolicy" {
  statement {
...
resource "aws_iam_role" "AmazonEKSLoadBalancerControllerRole" {
  name               = "${var.project_name}-${var.env}-AmazonEKSLoadBalancerControllerRole"
  assume_role_policy = data.aws_iam_policy_document.AmazonEKSLoadBalancerControllerAssumeRolePolicy.json
}

resource "aws_iam_policy" "AWSLoadBalancerControllerIAMPolicy" {
  name        = "${var.project_name}-${var.env}-AWSLoadBalancerControllerIAMPolicy"
  description = "${var.project_name}-${var.env}-AWSLoadBalancerControllerIAMPolicy"
  policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
